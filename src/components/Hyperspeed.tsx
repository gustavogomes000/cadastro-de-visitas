...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: EffectOptions }) => {
  ...
  useEffect(() => {
    ...
    class CarLights {
      webgl: any; options: any; colors: any; speed: number[]; fade: THREE.Vector2; mesh?: THREE.Mesh;
      constructor(webgl: any, options: any, colors: any, speed: number[], fade: THREE.Vector2) {
      ...
      init() {
        ...
        instanced.index = geometry.index; instanced.attributes = geometry.attributes;
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          const radius = random(options.carLightsRadius); const length = random(options.carLightsLength);
          const speed = random(this.speed); const carLane = i % options.lanesPerRoad;
          let laneX = carLane * laneWidth - options.roadWidth / 2 + laneWidth / 2;
          const carWidth = random(options.carWidthPercentage) * laneWidth;
          const carShiftX = random(options.carShiftX) * laneWidth; laneX += carShiftX;
          ...
          aColor.push(color.r, color.g, color.b); aColor.push(color.r, color.g, color.b);
        }
    ...
    class LightsSticks {
      ...
      init() {
        ...
        const instanced = new THREE.InstancedBufferGeometry();
        instanced.index = geometry.index; instanced.attributes = geometry.attributes;
        const totalSticks = options.totalSideLightSticks; instanced.instanceCount = totalSticks;
        ...
        for (let i = 0; i < totalSticks; i++) {
          const width = random(options.lightStickWidth); const height = random(options.lightStickHeight);
          aOffset.push((i - 1) * stickoffset * 2 + stickoffset * Math.random());
          const color = pickRandom(colors);
          aColor.push(color.r, color.g, color.b); aMetrics.push(width as number, height as number);
        }
    ...
    class Road {
      webgl: any; options: any; uTime: { value: number }; leftRoadWay?: THREE.Mesh; rightRoadWay?: THREE.Mesh; island?: THREE.Mesh;
      ...
      createPlane(side: number, _width: number, isRoad: boolean) {
        ...
        const material = new THREE.ShaderMaterial({
          ...
          side: THREE.DoubleSide, uniforms: Object.assign(uniforms, this.webgl.fogUniforms, options.distortion.uniforms)
        });
        material.onBeforeCompile = (shader: any) => { shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', options.distortion.getDistortion); };
    ...
    class App {
      options: any; container: HTMLElement; hasValidSize: boolean;
      renderer: THREE.WebGLRenderer; composer: EffectComposer;
      camera: THREE.PerspectiveCamera; scene: THREE.Scene;
      fogUniforms: any; clock: THREE.Clock; assets: any; disposed: boolean;
      ...
      constructor(container: HTMLElement, options: any) {
        ...
        const initW = Math.max(1, container.offsetWidth); const initH = Math.max(1, container.offsetHeight);
        this.renderer = new THREE.WebGLRenderer({ antialias: false, alpha: true });
        this.renderer.setSize(initW, initH, false); this.renderer.setPixelRatio(window.devicePixelRatio);
        ...
        this.scene = new THREE.Scene(); this.scene.background = new THREE.Color(options.colors.background);
        ...
        this.setSize = this.setSize.bind(this); this.onMouseDown = this.onMouseDown.bind(this);
        this.onMouseUp = this.onMouseUp.bind(this); this.onWindowResize = this.onWindowResize.bind(this);
      ...
      onWindowResize() {
        ...
        this.camera.updateProjectionMatrix(); this.composer.setSize(width, height); this.hasValidSize = true;
      }
      initPasses() {
      ...
      init() {
        this.initPasses(); const options = this.options;
        this.road.init();
      ...
      dispose() {
        ...
        if (this.scene) {
          this.scene.traverse((object: any) => {
            ...
            if (object.material) { if (Array.isArray(object.material)) object.material.forEach((m: any) => m.dispose()); else object.material.dispose(); }
          });
          this.scene.clear();
        }
        if (this.renderer) {
  ...
  return <div ref={hyperspeed} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', overflow: 'hidden' }} />;
};
